...
# --- [A. 환자용 서비스] ---
    ...
    # #1 피드백 반영: 제목 변경 및 스마트 라우팅 적용
        ...
        for i, name in enumerate(pharm_names):
            ...
            walk_t = get_walking_time(my_lat, my_lon, lats[i], lons[i])
            df_list.append({
                '약국명': name, 'lat': lats[i], 'lon': lons[i],
                '조제시간': eta_str, 'eta_raw': eta_raw, '도보시간': walk_t, '총소요시간': eta_raw + walk_t
            })
        
        df = pd.DataFrame(df_list).sort_values(by='총소요시간').reset_index(drop=True)
        df['id'] = range(1, 8)
    ...
    # #2 피드백 반영: 조제 산출식 유지 + 총 소요시간 산출식 추가
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.write(f"- 실시간 대기($N_{{wait}}$): {n_w}건")
            st.write(f"- 평균 조제 시간($T_{{avg}}$): {t_a}분")
            st.write(f"- 조제 인력($P_{{staff}}$): {p_s}명")
        with col2:
            st.write(f"- 시간 가중치($W_{{time}}$): {w_t}배")
            st.write(f"- 복잡 가중치($W_{{complex}}$): 1.1배")
            st.write(f"- 약국 유형 보정($B_{{type}}$): {b_t}분")
